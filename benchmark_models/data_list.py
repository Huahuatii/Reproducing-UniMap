...
pbmc9_hvg = pkl.load(open(os.path.join(root_data_folder, 'pbmc9/hvg_1754_pbmc9.pkl'), "rb"))
pbmc9_hvg = pkl.load(open(os.path.join(root_data_folder, 'pbmc9/hvg_1778_pbmc9.pkl'), "rb"))  # This is for remove B in ref, mast in query
pbmc9_hvg = pkl.load(open(os.path.join(root_data_folder, 'pbmc9/hvg_1772_pbmc9.pkl'), "rb"))  # This is for remove dc in ref, B in query
pbmc9_hvg = pkl.load(open(os.path.join(root_data_folder, 'pbmc9/hvg_1759_pbmc9.pkl'), "rb"))  # This is for remove B in ref, macrophage in query
pbmc9_hvg = pkl.load(open(os.path.join(root_data_folder, 'pbmc9/hvg_1750_pbmc9.pkl'), "rb"))  # This is for remove natural kill cell in ref, macrophage in query
pbmc9_hvg = pkl.load(open(os.path.join(root_data_folder, 'pbmc9/hvg_1802_pbmc9.pkl'), "rb"))  # This is for remove plasma in ref, macrophage in query
pbmc9_hvg = pkl.load(open(os.path.join(root_data_folder, 'pbmc9/hvg_1815_pbmc9.pkl'), "rb"))  # This is for remove plasma cell in ref, dendritic cell in query
...
def get_scanpy_adata(datatype, seed=2023, model='unimap'):
    ...
    if datatype == 'pbmc9':
        ...
        s_data = sc.read_h5ad(mg_t)
        t_data = sc.read_h5ad(mg_s)
        s_data = s_data[:, s_data.var.index.isin(mg_hvg)]
        t_data = t_data[:, t_data.var.index.isin(mg_hvg)]
...
def get_unimap_dataset(args):
    ...
    ce = Label_Encoder()
    ce.fit(list(set(s_celltype)))  # ? sparse警告来自于此  # acc相关
